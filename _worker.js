  ...
  async fetch(request, env) {
    ...
    async function onMessage(message) {
      ...
      if (text === '/start') {
        ...
        await sendMessageToUser(chatId, `👋 *欢迎使用Damon的传话筒*\n\n` +
              `📝 请使用礼貌用语进行对话\n` +
              `⏱ 管理员看到消息会及时回复\n\n` +
    ...
    async function getVerificationSuccessMessage() {
      ...
      const response = await fetch('https://raw.githubusercontent.com/damonwang127/TGChatBot/refs/heads/master/CFTeleTrans/start.md');
    ...
    async function getNotificationContent() {
      const response = await fetch('https://raw.githubusercontent.com/damonwang127/TGChatBot/refs/heads/master/CFTeleTrans/notification.md');
      ...
      if (action === 'verify') {
        ...
        if (action === 'block') {
          ...
          await env.D1.prepare('INSERT OR REPLACE INTO user_states (chat_id, is_blocked, is_first_verification) VALUES (?, ?, ?)')
            ...
            .run();
          await sendMessageToTopic(topicId, `✅ *操作成功*\nDamon把 \`${privateChatId}\` 从小黑屋中放了出来\n消息将继续转发`);
